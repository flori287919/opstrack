  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <CostContractForm action={update} initial={contract} projects={projects ?? []} submitLabel="Ruaj ndryshimet" />

        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl p-6">
            <h2 className="font-semibold text-slate-900 mb-3">Përmbledhje pagesash</h2>
            <div className="grid grid-cols-2 gap-4">
              <Stat label="Vlera kontratës" value={formatEUR(contract.contract_value_no_taxes)} />
              <Stat label="Të skeduluara" value={formatEUR(totalScheduled)} />
              <Stat label="Të paguara" value={formatEUR(totalPaid)} tone="green" />
              <Stat label="Mbeten" value={formatEUR(totalScheduled - totalPaid)} tone="amber" />
            </div>
          </div>

          <div className="bg-white border border-slate-200 rounded-xl">
            <div className="px-5 py-4 border-b border-slate-200">
              <h2 className="font-semibold text-slate-900">Pagesat</h2>
            </div>
            <div className="p-5">
              <form action={addPayment} className="grid grid-cols-2 gap-3 mb-5">
                ...
                <label className="text-sm">
                  <span className="block text-xs text-slate-600 mb-1">Due Date *</span>
                ...
                <label className="text-sm">
                  <span className="block text-xs text-slate-600 mb-1">Amount *</span>
                ...
                <label className="text-sm">
                  <span className="block text-xs text-slate-600 mb-1">% e kontratës</span>
              ...
              {(payments ?? []).length === 0 ? (
                <p className="text-sm text-slate-500 text-center py-4">Asnjë pagesë akoma.</p>
              ) : (
                      ...
                      <li key={p.id} className="py-3 flex items-center gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-slate-500 mt-0.5">
                            Due: {formatDate(p.due_payment_date)}
                            {p.actual_payment_date && ` • Paguar: ${formatDate(p.actual_payment_date)}`}
                          </div>
                        </div>
