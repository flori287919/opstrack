  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Kostot (Non-People)</h1>
          <p className="text-sm text-slate-500">
            {showDeleted ? 'Kontrata të fshira' : 'Kontrata aktive me subkontraktorë / furnitorë'}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Link href={showDeleted ? '/dashboard/costs' : '/dashboard/costs?show=deleted'} className="px-3 py-2 text-sm border border-slate-300 rounded-lg hover:bg-slate-100 text-slate-700">
            {showDeleted ? '← Aktive' : 'Të fshira'}
          </Link>
          {!showDeleted && (
            <Link href="/dashboard/costs/new" className="px-4 py-2 text-sm font-medium bg-slate-900 text-white rounded-lg hover:bg-slate-800">
              + Kontratë e re
            </Link>
          )}
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-slate-700">
              <th className="px-4 py-3 font-medium">Kontrata</th>
              <th className="px-4 py-3 font-medium">Subko</th>
              <th className="px-4 py-3 font-medium">Projekti</th>
              <th className="px-4 py-3 font-medium text-right">Vlera (pa taksa)</th>
            ...
            {(contracts ?? []).length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-slate-500">
                  {showDeleted ? 'Asnjë kontratë e fshirë.' : 'Asnjë kontratë akoma. Klik "Kontratë e re" për të nisur.'}
                </td>
              </tr>
            ) : (
              (contracts ?? []).map((c) => {
                  ...
                  <tr key={c.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      {showDeleted ? (
                        ...
                        <Link href={`/dashboard/costs/${c.id}`} className="text-slate-700 hover:underline">
                          Hap →
                        </Link>
                      )}
                    </td>
